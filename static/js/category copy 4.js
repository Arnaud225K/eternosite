...
//             resizeTimer = setTimeout(setupMobileView, 150);
//         });
//     }
...
//             const productList_item = productListContainer.querySelector('.subcategory__products-list');
//             if (productList_item && data.html_products) {
//                 const tempDiv = document.createElement('div');
//                 tempDiv.innerHTML = data.html_products;
//                 Array.from(tempDiv.children).forEach(card => productList_item.appendChild(card));
//             } else {
//                 console.error("[Load More] Could not find '#product-list-container' to append new products.");
//             }
...
//             // Ajouter uniquement les nouvelles cartes produits
...
//             // Remplacer le titre du compteur
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeFilterUI(forceMobileOpen = false) {
        ...
        filterContainer.querySelectorAll(".filters__box-item").forEach(item => { item.addEventListener("click", (e) => { if (e.target.closest(".filters__dropdown")) return; const wasOpen = item.classList.contains("open"); closeAllDropdowns(); if (!wasOpen) { item.classList.add("open"); const dropdown = item.querySelector(".filters__dropdown"); if (dropdown) dropdown.classList.remove("hidden"); } e.stopPropagation(); }); });
        document.addEventListener("click", (e) => { if (!e.target.closest(".filters__box-item")) closeAllDropdowns(); });
        filterContainer.querySelectorAll(".filters__search-input").forEach(searchInput => { searchInput.addEventListener('input', (e) => { e.stopPropagation(); const dropdown = searchInput.closest('.filters__dropdown'); const labels = dropdown.querySelectorAll("label"); const searchTerm = searchInput.value.toLowerCase().trim(); labels.forEach(label => { const text = label.textContent.toLowerCase().trim(); label.style.display = text.includes(searchTerm) ? 'flex' : 'none'; }); }); });
        const setupMobileView = () => { const title = filterContainer.querySelector(".filters__title"); const wrapper = filterContainer.querySelector(".filters__wrapper"); if (!title || !wrapper) return; const TRANSITION_DURATION = 400; if (window.innerWidth <= 692) { if (!title.classList.contains('filters__title--mobile')) { title.classList.add('filters__title--mobile'); } title.onclick = null; if (forceMobileOpen) { filterContainer.classList.add("is-open"); wrapper.style.maxHeight = wrapper.scrollHeight + "px"; setTimeout(() => { wrapper.style.overflow = "visible"; }, TRANSITION_DURATION); } else { wrapper.style.overflow = "hidden"; wrapper.style.maxHeight = "0px"; filterContainer.classList.remove("is-open"); } title.onclick = () => { const isOpen = filterContainer.classList.contains("is-open"); if (isOpen) { wrapper.style.overflow = "hidden"; wrapper.style.maxHeight = "0px"; filterContainer.classList.remove("is-open"); } else { wrapper.style.maxHeight = wrapper.scrollHeight + "px"; filterContainer.classList.add("is-open"); setTimeout(() => { wrapper.style.overflow = "visible"; }, TRANSITION_DURATION); } }; } else { wrapper.style.maxHeight = ""; wrapper.style.overflow = ""; filterContainer.classList.remove("is-open"); title.classList.remove('filters__title--mobile'); title.onclick = null; } };
        setupMobileView();
        let resizeTimer;
    ...
    function buildFilterUrlSegment() { const form = document.getElementById('filters-form'); if (!form) return ''; const activeFilters = {}; form.querySelectorAll('input[type="checkbox"]:checked').forEach(cb => { if (!activeFilters[cb.name]) activeFilters[cb.name] = []; activeFilters[cb.name].push(cb.value); }); const sortedKeys = Object.keys(activeFilters).sort(); if (sortedKeys.length === 0) return ''; const parts = sortedKeys.map(key => `${key}=${activeFilters[key].sort().join(',')}`); return `f/${parts.join('/')}/`; }
