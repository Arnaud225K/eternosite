...
from .managers import FilialManager
...
class Filial(models.Model):
    ...
    order_number = models.PositiveIntegerField("Порядок", help_text="Чем меньше число, тем выше в списке. Оставьте пустым для сортировки по алфавиту.", null=True, blank=True)
    ...
    robots_txt_override = models.TextField(
        "Переопределение Robots.txt",
        blank=True,
        help_text="Если заполнено, будет использоваться этот robots.txt для данного филиала. Иначе будет использоваться шаблон по умолчанию. Используйте {host} и {main_domain}."
    )
    sitemap_filename = models.CharField(
        "Имя файла Sitemap",
        max_length=100,
        blank=True,
        help_text="Например, 'sitemap_ekb.xml'. Если оставить пустым, будет использовано 'sitemap.xml'."
    )
