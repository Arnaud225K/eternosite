...
class ProjectSettings(SingletonModel):
	...
	main_map_code = models.TextField("Основной код карты", blank=True, help_text="HTML-код для вставки карты, например, с Яндекс.Карт.")
	main_working_hours = models.CharField("Основной режим работы", max_length=1024, blank=True, null=True)
	...
	robots_txt_default = models.TextField(
		"Robots.txt по умолчанию",
		blank=True,
		help_text="Шаблон по умолчанию. Используйте {host} для подстановки текущего домена (напр., ekb.mtk-novator.ru) и {main_domain} для основного домена (напр., mtk-novator.ru)."
	)

	def save(self, *args, **kwargs):
		cache.delete('project_settings_singleton')
...
class SocialLink(models.Model):
	...
	project_settings = models.ForeignKey(ProjectSettings, on_delete=models.CASCADE, related_name='social_links')
